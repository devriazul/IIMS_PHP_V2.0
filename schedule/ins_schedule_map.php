  ...
  if(($car['ins']=="y")||($_SESSION['userid']=="administrator")){
   ...
   $faculty=$farr[0];  // $facultyid=$farr[1];
	 ...
	 $tval=$myDb->select("select*from tbl_time_interval where yrpart='$yrpart' and (id not in(select interval_fid from tbl_schedule_map where routineforid='$routineforid' and dyid='$dyid' and yrpart='$yrpart' and mapyear='$mapyear')
      ...
      $tval_t=$myDb->select("select*from tbl_time_interval where yrpart='$yrpart' and (id not in(select interval_fid from tbl_schedule_map_teacher where facultyid='$faculty' and dyid='$dyid' and yrpart='$yrpart' and mapyear='$mapyear')
      ...
      $tval_st=$myDb->select("select*from tbl_time_interval where yrpart='$yrpart' and (id not in(select interval_fid from tbl_schedule_map_venue where vnuid='$vnuid' and dyid='$dyid' and yrpart='$yrpart' and mapyear='$mapyear')
	  ...
	  //generate section
	  if(!empty($section)){
		  $tval_t=$myDb->select("select*from tbl_time_interval where yrpart='$yrpart' and (id not in(select interval_fid from tbl_schedule_map_section where routineforid='$routineforid' and dyid='$dyid' and yrpart='$yrpart' and mapyear='$mapyear')
