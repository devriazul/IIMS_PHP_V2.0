...
{ 
  ...
  if(($car['ins']=="y")||($_SESSION['userid']=="administrator")){
   ...
   $sdq=$myDb->select("select*from tbl_schedule_map where vnuid='$id'");
   $sdqf=$myDb->get_row($sdq,'MYSQL_ASSOC');
   $sdq_t=$myDb->select("select*from tbl_schedule_map_teacher where vnuid='$id'");
   $sdqf_t=$myDb->get_row($sdq_t,'MYSQL_ASSOC');
     if(!empty($sdqf['id'])||!empty($sdqf_t['id'])){
	   ...
	   if($myDb->update_sql("DELETE FROM tbl_venue WHERE id='$id'")){
