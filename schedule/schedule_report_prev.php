...
{ 
  ...
  if(($car['ins']=="y")||($_SESSION['userid']=="administrator")){
	...
	if(!empty($_POST['rptYear'])){
		$mapyear=mysql_real_escape_string(!empty($_POST['rptYear'])?$_POST['rptYear']:'');
	}
...
<div style="width:500px; font-size:18px" align="center">Mirpur-6, Dhaka-1216<br/><span style="font-weight:bold;">Class Schedule-<?php echo $mapyear; ?></span></div>
</div>

  <?php 
  if(!empty($routineforid)){
  $tabrec=$myDb->select("select distinct f.name,f.contactno,d.code,c.section,s.name semestername,o.alias
  								from tbl_schedule_map_backup c
     ...
     <tr>
	   ...
	   <tr>
		 ...
		 $sdq=$myDb->select("select c.id,c.mapyear 'Year',d.code,c.section,s.name semname,rm.alias 'Routine Owner',rm.id routineforid,gm.facultyid guidefacultyid,
						...
						from tbl_schedule_map_teacher_backup c
						...
						$sdq=$myDb->select("select c.id,c.mapyear 'Year',d.code,c.section,s.name semname,rm.alias 'Routine Owner',rm.id routineforid,gm.facultyid guidefacultyid,
								   ...
								   ttm.intervalName tointerval,vm.id venuid,vm.venuname,vm.roomno,
								   fm.facultyid,fm.name 'Faculty Name',c.shortName,c.interval_fid,c.interval_toid,c.courseid
						from tbl_schedule_map_venue_backup c
						left join tbl_department d
						on d.id=c.deptid
						left join tbl_semester s
						on s.id=c.semesterid
						...
						?>
							 <?php if(($recf['interval_fid']!=$recf['interval_toid'])){ ?>
								...
								<?php if(!empty($facultyid)): ?>
									 <td valign="top" colspan="2" align="center"><?php echo $recf['coursecode']."<br/>".$recf['Routine Owner']."-".$snm."<br/>".$recf['roomno'].""; ?></td>
								
								<?php endif; ?>
								
								<?php if(!empty($vnuid)): ?>
									 <td valign="top" colspan="2" align="center"><?php echo $recf['coursecode']."<br/>".$recf['Routine Owner']."-".$snm."<br/>".$recf['shortName']; ?></td>
							 ...
							 <?php }elseif(($recf['interval_fid']==$recf['interval_toid'])){ ?>
								...
								<?php if(!empty($facultyid)): ?>
									 <td valign="top" align="center"><?php echo $recf['coursecode']."<br/>".$recf['Routine Owner']."-".$snm."<br/>".$recf['roomno'].""; ?></td>
								
								<?php endif; ?>
								
								<?php if(!empty($vnuid)): ?>
									 <td valign="top" align="center"><?php echo $recf['coursecode']."<br/>".$recf['Routine Owner']."-".$snm."<br/>".$recf['shortName']; ?></td>
