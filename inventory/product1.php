...
<p align="center" ><font face="Arial, Helvetica, sans-serif" size="2"><?php if(isset($_GET['msg'])){ echo $_GET['msg']; }?></font></p>
		<div id="top-search-div"> 
         ...
         <?php 
			   ...
			   $prdtype=urlencode($prdtype);
			   $prdtype=str_replace("+"," ",$prdtype);
			   $searchid=!empty($_POST['searchid'])?mysql_real_escape_string($_POST['searchid']):''; 
	if(!empty($searchid)){
	 		 $sdq="SELECT id,pname 'Product Name',packsize 'Pack Size',qty 'Qty', prtype 'Product Type' FROM tbl_product
					WHERE pname LIKE '$searchid%' order by id desc";
	
	
	}else if(!empty($prdtype)){
	 		 $sdq="SELECT id,pname 'Product Name',packsize 'Pack Size',qty 'Qty', prtype 'Product Type' FROM tbl_product
					WHERE prtype in(select storeid from tbl_store where storename='$prdtype') order by id desc";
	
	}else{
	
	 		 $sdq="SELECT id,pname 'Product Name',packsize 'Pack Size',qty 'Qty', prtype 'Product Type' FROM tbl_product
					WHERE pname LIKE '$searchid%' and prtype in(select storeid from tbl_store where storename='$prdtype')  order by id desc";
	}
