	 ...
	 $('#submit').click(function(){
	  ...
	  $('#price').focus(function(){
	   ...
	   $('#pid').keyup(function(){
	      ...
	      $.get('pick_product.php?p='+p+"&prtype="+prtype,function(rec){
		    $('#showpick').show();
		    $('#showpick').html(rec);
		  });	   
...
<form name="sfrm" id="sfrm" method="post">
  ...
  <tr>
    <td colspan="3" style="border-bottom:1px solid #999999; height:30px;">Edit Requesition </td>
  </tr>
  <tr>
    <td colspan="3">&nbsp;</td>
  </tr>
  <tr>
    <td width="166">Product Type </td>
    <td width="9">:</td>
    <td width="525"><label>
      <select name="prtype" id="prtype" onkeypress="return handleEnter(this, event)">
	   <option selected value="<?php echo $pft->rs['prtype']; ?>"><?php echo $pft->rs['prtype']; ?></option>
	   <?php $prt=$myDb->select("SELECT DISTINCT prtype FROM tbl_product ORDER BY prtype asc");
	   while($prtf=$myDb->get_row($prt,'MYSQL_ASSOC')){
	   ?>
	     <option value="<?php echo $prtf['prtype']; ?>"><?php echo $prtf['prtype']; ?></option>
	   <?php } ?>
	  </select>
	</label></td>
  </tr>  
  ...
  <tr>
    ...
    <td>:</td>
    <td><label>
<input type="date" name="expdate" id="expdate" size="30" value="<?php echo $pft->rs['reqdate']; ?>" onkeypress="return handleEnter(this, event)" >      
    </label></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
