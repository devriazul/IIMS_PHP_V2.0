...
{ 
  ...
  /*$chkpo=$myDb->select("SELECT*FROM tbl_purorder WHERE reqid='$reqid' and opby='$_SESSION[userid]' AND usestatus='FINP'");
  $chkpof=$myDb->get_row($chkpo,'MYSQL_ASSOC');
  
  
  $listpr=$myDb->select("SELECT p.*,c.pid,c.pprice,c.id buyid FROM tbl_product p inner join tbl_buyproduct c on p.id=c.pid where c.reqid='$reqid' and c.opby='$_SESSION[userid]'");
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" class="tbl_repeat" >
  ...
  <?php $bpr=mysql_query("select 
								 p.id prid,p.pname,
								 e.name 'Employee Name',
								 c.rqty 'Requisition Qty',
								 c.aqty 'Approve Qty',
								 c.appdate 'Approve Date',
								 c.pqty 'Purchase Qty',
								 c.pdate 'Purchase Date'
        from tbl_product p
