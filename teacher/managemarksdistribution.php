...
//getting number of rows and calculating no of pages
  if(isset($_GET['searchid'])){ 
    $sql = "select * from  tbl_marksdistribution WHERE markstype LIKE '%$_GET[searchid]%' and storedstatus<>'D' and departmentid='$row[deptid]'";  
	$rsd = mysql_query($sql);
    $count = mysql_num_rows($rsd);
    $pages = ceil($count/$per_page);

  }else{
    $sql = "select * from  tbl_marksdistribution where storedstatus<>'D' and departmentid='$row[deptid]'";
	$rsd = mysql_query($sql);
    $count = mysql_num_rows($rsd);
    $pages = ceil($count/$per_page);
  }	
...
$().ready(function() {
	$("#searchid").autocomplete("search_marksdistribution.php", {
		width: 260,
		matchContains: true,
		//mustMatch: true,
		//minChars: 0,
...
<script type="text/javascript">
   ...
   //Default Starting Page Results
	...
	$("#content1").load("marksdistribution_pagination.php?page=1", Hide_Load());
	...
	$("#pagination li").click(function(){
		...
		$("#content1").load("marksdistribution_pagination.php?page=" + pageNum, Hide_Load());
	});
...
<div id="top-search-div"> 
           <div id="content">
		   <label>Manage Marks Distribution </label>
		   <div class="input">
		   <form method="post" autocomplete="off" action="managemarksdistribution1.php">
